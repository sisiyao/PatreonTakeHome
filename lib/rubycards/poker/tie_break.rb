module TieBreak
  ...
  def set_card(n)
    @cards.find { |card| card_rank_count(card.rank) == n }
  end

  def cards_without(rank)
  ...
  def compare_full_house(other_hand)
    ...
    if three == 0
      ...
      if two == 0
        high_card = cards_without(set_card(3).rank) &
                    cards_without(set_card(2).rank)
        other_high_card = other_hand.cards_without(set_card(3).rank) &
                          other_hand.cards_without(set_card(2).rank)
    ...
    if high == 0
      ...
      if low == 0
        high_card = @cards.find do |card|
          card_rank_count(card.rank) != 2
        end
        other_high_card = other_hand.cards.find do |card|
          other_hand.card_rank_count(card.rank) != 2
        end
  ...
  def compare_set_then_high_card(n, other_hand)
    ...
    if set_card == other_set_card
      cards_without(set_card.rank).sort.last <=>
      other_hand.cards_without(set_card.rank).sort.last
    else
